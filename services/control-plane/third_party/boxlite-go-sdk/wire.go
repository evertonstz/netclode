...
type wireNetworkEnabled struct {
...
func enabledNetwork(allowNet []string) any {
	if allowNet == nil {
		allowNet = []string{}
	}
	return map[string]wireNetworkEnabled{"Enabled": {AllowNet: allowNet}}
}

func disabledNetwork() any {
	return "Disabled"
}
...
func buildOptionsJSON(image string, cfg *boxConfig) (boxOptionsWire, error) {
	...
	network := NetworkSpec{Mode: NetworkModeEnabled}
	if cfg.network != nil {
		network = *cfg.network
	}
	...
	case "", NetworkModeEnabled:
		w.Network = enabledNetwork(allowNet)
